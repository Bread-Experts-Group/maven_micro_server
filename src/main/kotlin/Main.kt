...
fun main(args: Array<String>) {
	...
	val mavenStores = multipleArgs.getValue("store").map { File(it as String) }
	while (true) {
				...
				when (request.method) {
					HTTPMethod.GET, HTTPMethod.HEAD -> mavenStores.firstOrNull {
						...
						if (requestedPath.exists()) {
							debug("Found file for \"$storePath\" at \"${requestedPath.absolutePath}\"")
							HTTPResponse(
								200, request.version,
								mapOf(
									"Content-Type" to "application/octet-stream",
									"Content-Length" to requestedPath.length().toString()
								)
					...
					HTTPMethod.PUT -> {
						...
						if (size == null || size < 1) {
							...
							mavenStores.forEach {
								...
								if (writtenFile == null) {
									info("New file [$size] written for \"$storePath\" at \"${requestedPath.absolutePath}\"")
									var remainder = size
									while (remainder > 0) {
									...
									debug(
										"File [$size] for \"$storePath\" copied to " +
												"\"${requestedPath.absolutePath}\" from \"${writtenFile.absolutePath}\""
									)
